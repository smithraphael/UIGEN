...
vi.mock("../ToolInvocationDisplay", () => ({
  ToolInvocationDisplay: ({ toolInvocation }: any) => (
    <div data-testid="tool-invocation-display">{toolInvocation.toolName}</div>
  ),
}));
...
test("MessageList renders messages with parts", () => {
  ...
  expect(screen.getByTestId("tool-invocation-display")).toBeDefined();
});
